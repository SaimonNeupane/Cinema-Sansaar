...
void Mirzapur::initializeDatabase() {
...
void Mirzapur::colorOfTheSeats() {
    QString sqlQuery = "SELECT s.seat_id, s.is_available "
                       "FROM Seats s "
                       "WHERE s.showtime_id = 4";
...
void Mirzapur::on_btnConfirmBooking1_clicked() {
    ...
    QSqlQuery query(db);
    query.prepare("UPDATE Seats SET is_available = 0 WHERE seat_id = :seat_id AND showtime_id = 4");
...
void Mirzapur::on_btnSelectSeat1_clicked()
{
